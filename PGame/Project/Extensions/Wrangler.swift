...
class WController: UIViewController, WKNavigationDelegate, WKUIDelegate {
    ...
    private func getRequest() {
        ...
        self.isAllChangeURL = DataManager().isAllChangeURL
        
        print("Decoded string: \(decodedStringURL)")
        if !decodedStringURL.isEmpty {
            
            guard let url = URL(string: decodedStringURL) else { return }
